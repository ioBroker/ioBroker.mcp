...
class McpServer {
    ...
    createServer(subscriptions) {
        ...
        server.registerTool('get_logs', {
            description: 'Retrieve recent ioBroker log lines from the host log file. Each entry has {ts, level, ' +
        ...
        server.registerTool('search_adapter_repository', {
            description: 'Search the ioBroker adapter REPOSITORY (all installable adapters, not just the ' +
            ...
            inputSchema: {
                query: zod_1.z.string().describe('Keyword to search for, e.g. "philips hue", "zigbee", "mqtt"'),
                type: zod_1.z
                    .string()
                    .optional()
                    .describe('Filter by adapter category/type, e.g. "lighting", "climate-control", "hardware"'),
                onlyNotInstalled: zod_1.z
                    .boolean()
                    .optional()
                    .describe('If true, return only adapters that are not installed yet'),
                language: zod_1.z.enum(LANGUAGES).optional().describe('Language for title/description'),
    ...
    getLogs(params) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.adapter.sendToHost(this.adapter.host || null, 'getLogs', fetchLines, (result) => {
    ...
    async pingHost(params) {
        ...
        this.adapter.log.debug(`[ping_host] request host="${host}" port=${params.port ?? '-'} count=${count} timeout=${timeout}ms platform=${process.platform}`);
    ...
    icmpPing(host, count, timeout) {
        ...
        return new Promise(resolve => {
            (0, node_child_process_1.execFile)(bin, args, { timeout: timeout * count + 3000, windowsHide: true }, (err, stdout, stderr) => {
                const out = `${stdout || ''}${stderr || ''}`;
                const errCode = err ? `${err.code ?? err.message}` : 'none';
                ...
                if (err && err.code === 'ENOENT') {
                    ...
                    this.adapter.log.warn(`[ping_host] "ping" executable not found. ICMP check skipped — using TCP fallback. To enable ICMP, install it: ${install.command}`);
                    resolve({
                        reachable: false,
                        unavailable: true,
                ...
                const avg = isWin
                    ...
                    : out.match(/=\s*[\d.]+\/([\d.]+)\//)?.[1];
                this.adapter.log.debug(`[ping_host] icmp parsed: received=${received} (matchedWin=${isWin}) avg=${avg ?? '-'} -> reachable=${received > 0}`);
                resolve({
     ...
     */
    async tcpReachable(host, timeout) {
    ...
    async searchAdapterRepository(params) {
        ...
        const sources = (activeWithJson.length ? activeWithJson.map(name => repositories[name]) : Object.values(repositories))
            .map(repo => repo?.json)
            .filter((j) => !!j);
        const json = {};
        ...
        const installedView = await this.adapter.getObjectViewAsync('system', 'adapter', { startkey: 'system.adapter.', endkey: 'system.adapter.香' }, { user: this.defaultUser });
        const installed = new Set(installedView.rows.map(row => row.value?.common?.name || row.id.replace('system.adapter.', '')));
        const results = [];
